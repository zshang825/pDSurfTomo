...
..
    ...
    modified by Shaohang Zhu (USTC)
    zshang@mail.ustc.edu.cn
    Vectorization for the forward modeling of surface wave dispersion
...
def sphere(ifunc, d, a, b, rho):
    ...
    for i in range(len(d)):
        ...
        if ifunc == 1:
            ...
            rho[i] = rho[i] * btp[i] ** (-2.275)
    thick[-1] = 0.0


@jitted
def surf96(t, d, a, b, rho, iflsph = 0, mode = 0, itype = 0, ifunc = 2, dc = 0.005, dt = 0.025):
    """
...
def surf96_guvectorize(t, d, a, b, rho, iflsph, mode, itype, ifunc, dc, dt, out):
    """
    Function Signature: (n_periods), (n_layers), (n_layers), (n_layers), (n_layers), scalars... -> (n_periods)
    """
    if iflsph == 1:
        d = np.copy(d)
        a = np.copy(a)
...
def surf96_batch(t_batch, d_batch, a_batch, b_batch, rho_batch,
                 iflsph = 0, mode = 0, itype = 0, ifunc = 2, dc = 0.005, dt = 0.025):
    """
    t_batch: [batch_size, n_periods]
    d_batch, a_batch, b_batch, rho_batch: [batch_size, n_layers]
    """
    return surf96_guvectorize(t_batch, d_batch, a_batch, b_batch, rho_batch, iflsph, mode, itype, ifunc, dc, dt)
