subroutine depthkernel(nx, ny, nz, vel, pvRc, sen_vsRc, sen_vpRc, sen_rhoRc, &
    ...
    do jj = 1, ny
        do ii = 1, nx
            ...
            do i = 1, mmax
                ...
                call refineGrid2LayerMdl(minthk, mmax, depm, vpm, vsm, rhom, rmax, rdep, rvp, rvs, rrho, rthk)
                call surfdisp96(rthk, rvp, rvs, rrho, rmax, iflsph, iwave, mode, igr, kmaxRc, tRc, cg1)

                vsm(i) = vsz(i) + 0.5*dlnVs*vsz(i)
                call refineGrid2LayerMdl(minthk, mmax, depm, vpm, vsm, rhom, rmax, rdep, rvp, rvs, rrho, rthk)
                call surfdisp96(rthk, rvp, rvs, rrho, rmax, iflsph, iwave, mode, igr, kmaxRc, tRc, cg2)
                vsm(i) = vsz(i)
    ...
    ! call WriteArray_binary_2D(pvRc, get_path("dir") //"/DepthK/pvRc_" // ts(Iteration) // ".bin")
    ! call WriteArray_binary_3D(sen_vpRc, get_path("dir") //"/DepthK/sen_vpRc_" // ts(Iteration) // ".bin")
    ! call WriteArray_binary_3D(sen_vsRc, get_path("dir") //"/DepthK/sen_vsRc_" // ts(Iteration) // ".bin")
    ! call WriteArray_binary_3D(sen_rhoRc, get_path("dir") //"/DepthK/sen_rhoRc_" // ts(Iteration) // ".bin")
...
subroutine CalSurfG(nx, ny, nz, nparpi, vels, iw, rw, col, dsurf, &
    ...
    real:: loop_use_time, travel_total_time, srtimes_total_time, rpath_total_time
    ...
    if (kmaxRg .gt. 0) then
        iwave = 2
        igr = 0
        call caldespersion(nx, ny, nz, vels, pvRc, iwave, igr, kmaxRg, tRg, depz, minthk)
        igr = 1
        write (*, "(A)") "Rayleigh wave group velocity depth kernel"
        call depthkernel(nx, ny, nz, vels, pvRg, sen_vsRg, sen_vpRg, sen_rhoRg, iwave, igr, kmaxRg, tRg, depz, minthk)
    end if
    ...
    do knumi = 1, kmax
        do srcnum = 1, nsrcsurf1(knumi)
            ...
            do ig = 1, igroup
                ...
                !
                ! call date_and_time(values=start_date_travel)
                urg = 0
                IF (asgr .EQ. 1) THEN
                    !
                ...
                ELSE
                    ...
                    CALL travel(x, z, urg)
                END IF
                ! call date_and_time(values=end_date_travel)
                ! func_name = ""
                ! call cal_run_time(start_date_travel, end_date_travel, func_name, loop_use_time)
                ! travel_total_time = travel_total_time + loop_use_time
                !
                !  Find source-receiver traveltimes if required
                !
                !
                do istep = 1, nrc1(srcnum, knumi)
                    ! call date_and_time(values=start_date_srtimes)
                    if (ig == 1) then
                        CALL srtimes(x, z, rcxf(istep, srcnum, knumi), rczf(istep, srcnum, knumi), cbst1)
                        count1 = count1 + 1
                        dsurf(count1) = cbst1
                    end if
                    ! call date_and_time(values=end_date_srtimes)
                    ! func_name = ""
                    ! call cal_run_time(start_date_srtimes, end_date_srtimes, func_name, loop_use_time)
                    ! srtimes_total_time = srtimes_total_time + loop_use_time                    
                    ! -------------------------------------------------------------
                    ! ENDIF
                    !
                    ...
                    ! if required.
                    !
                    ! call date_and_time(values=start_date_rpath)
                    ...
                    ! call date_and_time(values=end_date_rpath)
                    ! func_name = ""
                    ! call cal_run_time(start_date_rpath, end_date_rpath, func_name, loop_use_time)
                    ! rpath_total_time = rpath_total_time + loop_use_time
                end do
...
subroutine refineGrid2LayerMdl(minthk0, mmax, dep, vp, vs, rho, rmax, rdep, rvp, rvs, rrho, rthk)
    ! --------------------------------------------------------------------c
...
subroutine synthetic(nx, ny, nz, nparpi, vels, obst, &
    ...
    integer :: start_date_rpath(8), end_date_rpath(8)
    integer :: start_date_srtimes(8), end_date_srtimes(8)
    real:: loop_use_time, travel_total_time, srtimes_total_time
    ...
    if (kmaxRc .gt. 0) then
        ...
        call caldespersion(nx, ny, nz, vels, pvRc, iwave, igr, kmaxRc, tRc, depz, minthk)
    ...
    if (kmaxRg .gt. 0) then
        iwave = 2
        igr = 1
        call caldespersion(nx, ny, nz, vels, pvRg, iwave, igr, kmaxRg, tRg, depz, minthk)
    ...
    if (kmaxLc .gt. 0) then
        iwave = 1
        igr = 0
        call caldespersion(nx, ny, nz, vels, pvLc, iwave, igr, kmaxLc, tLc, depz, minthk)
    ...
    if (kmaxLg .gt. 0) then
        iwave = 1
        igr = 1
        call caldespersion(nx, ny, nz, vels, pvLg, iwave, igr, kmaxLg, tLg, depz, minthk)
...
subroutine caldespersion(nx, ny, nz, vel, pvRc, iwave, igr, kmaxRc, tRc, depz, minthk)
    use omp_lib
    use ParallelConfig
    implicit none
    ...
    do jj = 1, ny
        do ii = 1, nx
            ...
            call refineGrid2LayerMdl(minthk, mmax, depz, vpz, vsz, rhoz, rmax, rdep, rvp, rvs, rrho, rthk)
